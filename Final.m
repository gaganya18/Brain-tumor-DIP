...
%erosion the wall by a few pixels
dilationAmount = 5;
...
%% Display Together
figure
subplot(231);imshow(s);title('Input image','FontSize',14);
subplot(232);imshow(inp);title('Filtered image','FontSize',14);
subplot(233);imshow(inp);title('Bounding Box','FontSize',14);
hold on;rectangle('Position',wantedBox,'EdgeColor','y');hold off;
subplot(234);imshow(tumor);title('tumor alone','FontSize',14);
subplot(235);imshow(tumorOutline);title('Tumor Outline','FontSize',14);
subplot(236);imshow(tumorOutlineInserted);title('Detected Tumor','FontSize',14);
